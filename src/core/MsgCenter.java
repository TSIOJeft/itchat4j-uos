...
import com.farplace.farpush.itchat.api.MessageTools;
...
import com.farplace.farpush.itchat.utils.enums.MsgCodeEnum;
...
public class MsgCenter {
    ...
    public static void handleMsg(IMsgHandlerFace msgHandler) {
        new Thread(() -> {
            while (true) {
                if (core.getMsgList().size() > 0 && core.getMsgList().get(0)!=null&&core.getMsgList().get(0).getContent() != null) {
                    if (core.getMsgList().get(0).getContent().length() > 0) {
                        BaseMsg msg = core.getMsgList().get(0);
                        if (msg == null) return;
                        if (msg.isGroupMsg() && !core.receive_group) continue;
                        if (msg.getType() != null) {
                            try {
...
//								MessageTools.sendMsgById(result, core.getMsgList().get(0).getFromUserName());
                    ...
                    try {
                        Thread.sleep(Config.SYNC_MSG * 1000L);
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                }
